...
export async function startHotKeeper(entryFile, config) {
  ...
  let server = null;
  const entryDirname = dirname(entryFile);
  ...
  const clearRequireCache = () => {
    ...
    Object.keys(require.cache).forEach((key) => {
      const shouldExclude = excludePaths.some((excludePath) =>
        key.includes(resolve(entryDirname, excludePath))
      );
      ...
      const shouldClear = watchPaths.some((watchPath) =>
        key.includes(resolve(entryDirname, watchPath))
      );
    ...
    try {
      ...
      if (config.secure) {
        const certPath = resolve(
          entryDirname,
          config.certs?.cert || "./certs/cert.pem"
        );
        const keyPath = resolve(
          entryDirname,
          config.certs?.key || "./certs/key.pem"
        );
  ...
  const resolvedWatchPaths = watchPaths.map((path) =>
    resolve(entryDirname, path)
  );
